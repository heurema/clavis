...
func TestUsersArgumentsRejectedBeforeIO(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{args: []string{"users", "block", "--user=INVALID", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "unblock", "--user=7fde7ce1-cc8d-4de8-a9c0", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "reset-password", "--user=INVALID", "--password-stdin", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "reset-password", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "set-role", "--user", id, "--role=owner", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "set-role", "--user", id, "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "set-role", "--role=admin", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "list", "extra"}},
		{args: []string{"users", "block", "--user", id, "extra"}},
		{args: []string{"users", "create", "--password-stdin", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "create", "--username=UPPER", "--password-stdin", "--server=http://127.0.0.1:1"}},
		{args: []string{"users", "create", "--username=alice", "--password=value"}},
		{args: []string{"users", "create", "--username=alice", "--server=http://127.0.0.1:1"}, noninteractive: true},
		{args: []string{"users", "reset-password", "--user", id, "--server=http://127.0.0.1:1"}, noninteractive: true},
		{args: []string{"users", "unknown"}},
		{args: []string{"users", "list", "--timeout=0", "--server=http://127.0.0.1:1"}},
