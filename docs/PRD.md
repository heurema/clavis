...
The MVP needs a sign-in screen plus read-only administrative views of users, connections and permissions. The browser does not offer management forms in the MVP; every mutation goes through the CLI. There is no status page: readiness is reported by the JSON endpoint `GET /health/ready` and by `clavis doctor`.
