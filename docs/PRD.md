...
Each user signs in with a personal account. An administrator grants access to connections configured in advance. The agent accesses external systems through the platform with the user's permissions. The platform checks access to the connection, executes a supported operation and returns the result.
...
> Ask your agent a question: it will retrieve the company data you are allowed to access and help you complete the task. Access is centrally managed.
...
Positioning (September 13, 2026): Clavis is the access model Grafana keeps inside its data source proxy, rebuilt for agents, where the identity behind every request is the human, not a shared service account, and with bounds designed for a caller that loops.
...
| Administrator | Manage users, connections, and permissions | Centralized control |
...
| Data storage | Platform management data only; no audit journal in the MVP and no persistent storage of external query results or query text |
| Audit journal | Not in the MVP (September 12, 2026); requests and administrative changes are not recorded; a journal with retention is a later stage |
...
An operation is a supported action through a provider or an administrative action in the platform. The MVP does not keep a journal of operations; an audit journal is a later stage.
...
The administrator configures restricted access in the external system before granting a connection to users. The platform provides a unified way to use that access.
...
All users of a connection share its preconfigured external access scope. The external source may see a shared role or token; the platform knows which user made each request but does not record it in the MVP.
...
| CONN-05 | Credential updates | An administrator can replace credentials without exposing secrets |
| CONN-06 | Protected credential handling | Secrets are encrypted at rest with a deployment-supplied key, are not returned to members or agents when viewing connections, and are excluded from error messages and logs |
| CONN-07 | Extensible provider support | A new system type uses the shared user, connection and permission model |
...
Implemented on September 14, 2026: the skill is embedded in the CLI binary and installed with `clavis skill install` into the Claude Code and Codex skill directories present on the machine (`~/.claude/skills/clavis`, `~/.codex/skills/clavis`, or a project's `.claude/skills` and `.codex/skills`), stamped with the CLI version; a test fails the build when the skill names a command, flag or hint the CLI lacks. It is one `SKILL.md` entry plus one reference per provider, teaching discovery before querying, the bounds and the failure codes of each source.
...
**No audit journal in the MVP** (decided September 12, 2026): the platform does not record operations, administrative changes or denied attempts. Attribution of requests to users happens at request time through the authenticated session and is not persisted. An audit journal with request events (operation, user, session, connection, timestamp, duration, outcome, result size and truncation, never query text) and administrative change events is a later stage.

**MVP storage rule:** SQL query results, time series, and other source responses are not stored persistently. This applies to service logs and platform error diagnostics. Query text and arbitrary operation parameters are never stored, because they may contain sensitive values.
...
6. The manager receives an answer with its source and time range.
...
3. The platform checks permissions and applies the change.
...
4. The platform returns the source's denial clearly.
...
- Connection access assignments to users or groups, with effective access as their union; management is the administrator role.
...
- Request proxying without stored query text.
...
1. **Local sign-in:** an administrator creates a local account, the user signs in through the browser and approves the CLI from there, and an administrator can block the user, reset the password and revoke sessions.
...
8. **Auditing:** not in the MVP; every operation is attributed to the authenticated user at request time, and a persistent journal is a later stage.
9. **Data handling:** credentials are encrypted at rest and absent from logs; external system responses and query text are never persisted.
...
Numerical targets for performance and user metrics will be defined around the pilot team's circumstances. Permission enforcement and the absence of secrets in logs are mandatory acceptance criteria.
...
| Browser sign-in from the CLI | Revised on September 17, 2026 by the owner: signing in happens only in the browser. `clavis login` opens `/authorize`, the user signs in there if needed and clicks Approve, which creates the one-time code and redirects to the CLI's loopback callback; the CLI exchanges the code with its PKCE verifier for an ordinary session. `login` takes no `--username` or `--password-stdin` and the JSON password route `POST /api/auth/login` is removed, so a password is entered only on the sign-in page and an older CLI cannot sign in; `--no-browser` prints the link for another browser or an `ssh -L` tunnel; refresh tokens and device flow rejected or deferred |
| Session lifetime | Settled on September 17, 2026 by the owner: one policy for browser and CLI sessions, an idle expiry of seven days (`CLAVIS_SESSION_IDLE_TIMEOUT`, default `168h`) and an absolute cap of thirty days (`CLAVIS_SESSION_MAX_LIFETIME`, default `720h`), with `5m` ≤ idle ≤ max ≤ `2160h`; use renews the idle expiry only once less than half the idle window remains and never past the cap; replaces the fixed eight-hour lifetime, and the migration revokes every existing session |
...
The technical stack is Go for the backend and CLI, with templ, templUI and Tailwind CSS for the minimal web interface, sqlc-generated pgx application queries and Goose SQL migrations. The API, migrations and web assets are delivered in one Go server executable; PostgreSQL remains external and the CLI remains separate. Implemented so far: automated initial administrator setup, local browser/CLI sign-in, revocable sessions, administrator-managed local users with the peer administrator model and self-protection, registered connections with encrypted credentials, labels, resource bounds and connectivity checks for every provider, connection grants with member visibility, the per-request authorization check the providers call, users addressable by UUID or username, administrator-managed groups with membership, grants to a group, effective access as the union of direct and group grants, the provenance listing behind `grants list --effective` and the group names in `whoami`, and administration pages for users, groups, connections and grants with read-only lists inside one shell; the audit journal built by the earlier changes was removed on September 12, 2026, and the public setup/status page with its HTML readiness fragment was removed on September 14, 2026; PostgreSQL pass-through query execution with the per-connection timeout, row and byte caps, structured results and distinguishable source failures; VictoriaMetrics pass-through for PromQL instant and range queries and metadata discovery in the source's own format under the same bounds; VictoriaLogs pass-through for LogsQL and log discovery with the source's limit forwarded as typed and the unbounded stream cut at the cap; the agent skill embedded in the CLI and installed with `clavis skill install`, with a drift test binding its prose to the CLI; groups landed on September 14, 2026, completing the MVP access model. A dedicated schema-discovery command was dropped on September 12, 2026: agents discover schema through the pass-through itself, and the skill documents the catalog query to run first. Packaged delivery followed on September 15, 2026: the server ships as a non-root container image and a Helm chart that takes the database URL, the encryption key and the bootstrap password as Secret references, verified locally by an image smoke against the compose database, a chart lint with schema validation, and a kind run that installs the chart, signs in over a port-forward and upgrades across a migration. Tagged releases followed on September 16, 2026: pushing a `vX.Y.Z` tag, or a `vX.Y.Z-rc.N` candidate, runs the quality check and publishes the CLI for macOS and Linux on both architectures with checksums on a GitHub Release, the server image at `ghcr.io/heurema/clavis` and the chart at `oci://ghcr.io/heurema/charts/clavis`; every build reports its identity, with `go install` at a tag and a downloaded binary both naming the release version. Browser sign-in for the CLI and session renewal followed on September 17, 2026: `clavis login` is approved in the browser and the JSON password route is gone, so a password is entered only on the sign-in page, and browser and CLI sessions renew while they are used under an idle timeout and an absolute lifetime.
