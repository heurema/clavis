...
- `internal/provider`: the registry. `type Provider interface { Name() string; ParseTarget(raw TargetInput) (Target, error); ValidateSecret(Secret) error; Probe(ctx, Target, Secret) Outcome }`. Two implementations: `postgres.go` (pgx, `SELECT 1`) and `victoriametrics.go` (`net/http`, GET `/health`, header from auth method). `Target` is a provider-neutral struct with a validated `map[string]string` of non-secret settings and a canonical `URL` string; `Secret` is `auth.Secret`. The package imports neither database nor server code.
...
`{connectionID}` is a UUID or a name; the route validates it as one of the two shapes before the service call. Errors: 400 `INVALID_ARGUMENT`, 401, 403, 404 `CONNECTION_NOT_FOUND`, 409 `CONNECTION_EXISTS` / `CONNECTION_IN_USE`, 409 `CREDENTIALS_UNAVAILABLE`, 503. Every error may carry `hint`. `auditRejection` maps the routes to `connection.*` actions; pre-service rejections on update/credentials/enable/disable/delete use their own action; the check route maps to `connection.check`.
