...
The execution capability is an optional `provider.Executor` interface (`Execute(ctx, target, secret, request ExecuteRequest) (ExecuteResult, error)`) that PostgreSQL implements and VictoriaMetrics does not; the service decides `PROVIDER_UNSUPPORTED` by a type assertion, so nothing is called or opened for a provider without it with `ExecuteRequest{SQL string; Timeout time.Duration; MaxRows int; MaxBytes int}` and `ExecuteResult{Results []auth.QueryResult; Truncated bool; Statements, Rows, Bytes int64}`. Errors are typed: `*SourceError{auth.SourceFailure}`, `ErrTimeout`, `ErrUnreachable`, `ErrAuthRejected`, `ErrUnsupported`. VictoriaMetrics returns `ErrUnsupported`.
...
- [Statement timeout does not cover connect time] → the dial budget is capped at the operation timeout, so an unreachable host fails within seconds; the hung-connection backstop covers a connection that stops answering mid-request.
