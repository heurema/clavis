...
The system SHALL persist safe bootstrap, sign-in, session-administration, user-administration, connection-management, grant and query-execution events, including denied/failed attempts when storage is available. Events SHALL contain only documented identifiers and outcome metadata, not submitted unknown usernames, passwords, password hashes, connection secrets, target hosts, session tokens/digests, cookies, request bodies or raw errors. Successful mutations and their events SHALL commit together. Storage failure SHALL NOT be represented as successful durable auditing.

The event action allowlist SHALL be `bootstrap`, `login`, `logout`, `revoke`, `user.create`, `user.block`, `user.unblock`, `user.reset_password`, `user.promote`, `user.demote`, `users.list`, `connection.create`, `connection.update`, `connection.set_credentials`, `connection.enable`, `connection.disable`, `connection.delete`, `connection.check`, `connection.get`, `connections.list`, `grant.create`, `grant.revoke`, `grants.list` and `query.execute`. The outcome allowlist SHALL add `connection_disabled`, `truncated`, `source_error`, `source_timeout`, `source_unreachable` and `source_auth_rejected` to the existing outcomes. Events SHALL have an optional connection identifier column so an event may reference a user as its target and a connection at the same time, and optional integer columns for duration in milliseconds, statements executed, rows returned and bytes returned that only request events fill; existing rows keep them null. Extending either allowlist or the columns SHALL use a new forward Goose migration that replaces the check constraints without rewriting applied migrations or existing rows. User-administration events SHALL record the actor UUID, the actor's session UUID and the target user UUID when known; a created user's UUID SHALL be the target of its creation event. Connection events SHALL record the actor UUID, the actor's session UUID and the connection UUID as the target when known; a created connection's UUID SHALL be the target of its creation event. Grant events SHALL record the actor, the actor's session, the user as the target and the connection in the connection column. Query-execution events SHALL record the actor, the actor's session, the connection in the connection column, the outcome and the request metrics, and SHALL be written after the external request in their own transaction; they SHALL never contain SQL text, source messages or values. Dry runs SHALL record no event.
...
#### Scenario: Query execution is recorded
- **WHEN** a user executes a query, or a member's execution is denied
- **THEN** one `query.execute` event records the actor, session, connection, outcome, duration, statement count and returned rows and bytes when known, and no event contains the SQL or a source message
