...
- **Database:** migration `008` revokes all sessions and adds `sessions.max_expires_at` with a check that `expires_at ≤ max_expires_at`, an index on `sessions(expires_at)`, and the `cli_authorizations` table with its expiry index. New and changed queries in `internal/database/queries/auth.sql`, called from `internal/database/limits.go`, with sqlc regenerated.
- **Server:** `internal/database/auth.go` (renewal, cleanup, code issuance and exchange, the `NewLocalAuth` signature), `internal/auth/contracts.go` (the session duration constants, `LoginPath` replaced by the token path, the `IdleExpiresAt` field, the service methods), `internal/config/config.go`, `internal/server/auth.go` (authorize page, approve POST, token route, `next` on login, `loginJSON` removed), `internal/web` (the authorize and invalid-link documents, the login model's return target).
